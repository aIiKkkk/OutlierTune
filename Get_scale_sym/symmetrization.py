...
@torch.no_grad()
def symmetrization_ln_fcs(ln, fcs, act_scales, hyperparameters):
    ...
    if hyperparameters:
        scales_max = torch.max(scales)
        threshold =  scales_max * hyperparameters
        scales[scales < threshold] = 0
...
@torch.no_grad()
def symmetrization_lm(model, scales, hyperparameters):
    for name, module in model.named_modules():
        if isinstance(module, OPTDecoderLayer):
            ...
            symmetrization_ln_fcs(attn_ln, qkv, qkv_input_scales, hyperparameters)
            ffn_ln = module.final_layer_norm
            fc1 = module.fc1
            fc1_input_scales = scales[name + '.fc1']["input"]
            fc1_input_scales = fc1_input_scales / 512
            symmetrization_ln_fcs(ffn_ln, fc1, fc1_input_scales, hyperparameters)
