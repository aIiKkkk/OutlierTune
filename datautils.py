...
def get_wikitext2(nsamples, seed, seqlen, model, cache_dir):
    ...
    traindata = load_dataset('wikitext', 'wikitext-2-raw-v1', cache_dir='/data/wangjinguang/dataset/wikitext/', split='train')
    testdata = load_dataset('wikitext', 'wikitext-2-raw-v1', cache_dir='/data/wangjinguang/dataset/wikitext/', split='test')
...
def get_ptb(nsamples, seed, seqlen, model,cache_dir):
    print("get_ptb")
    from datasets import load_dataset
    traindata = load_dataset('ptb_text_only', 'penn_treebank',cache_dir='/data/wangjinguang/dataset/ptb_text_only/', split='train')
    valdata = load_dataset('ptb_text_only', 'penn_treebank',cache_dir='/data/wangjinguang/dataset/ptb_text_only/', split='validation')
...
def get_c4(nsamples, seed, seqlen, model, cache_dir):
    print("get_c4")
    from datasets import load_dataset
    valdata = load_dataset('json', data_files="/data/wangjinguang/dataset/c4-05-10/c4-validation.00000-of-00008.json.gz", split='train')
